...
p.range=prctile(m,[50+[-1 1]*p.range/2 0 100]); %first 2 %SIZE(4,5) FOR EACH PARAMETER PRC 0.25 99.75 0 100
rng=p.range(4,:)-p.range(3,:);                  %RANGE 0-100
if isempty(p.support),p.support=nan(2,M);end
ix=isnan(p.support(1,:));
p.support(1,ix)=p.range(3,ix)-rng(ix)/4;
ix=isnan(p.support(2,:)); 
p.support(2,ix)=p.range(4,ix)+rng(ix)/4;  %GENEROUS RANGE FOR EACH PARAMETER OPTIMIZED
...
clf
H=nan(M);
for r=1:M                %FOR EACH PARAMETER
    for c=1:max(r,M*p.fullmatrix)      %LOOP FROM 1:1,1:2,1:3 ETC
        ...
        if r==M, xlabel(['^{ }' p.names{c} '_{ }'],'FontSize',8,'FontWeight','Bold');end
